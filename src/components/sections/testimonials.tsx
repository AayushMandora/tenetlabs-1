  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: count }).map((_, i) => (
        <svg key={i} className="h-4 w-4 text-[#10b981]" viewBox="0 0 20 20" fill="currentColor">
...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-none bg-[#064e3b] text-sm font-bold text-[#10b981]">
      {initials}
    </div>
  );
}
        ...
        {siteContent.testimonials.items.map((testimonial) => (
          <StaggerItem key={testimonial.name} className="h-full">
            <article className={cn("group relative flex h-full flex-col overflow-hidden rounded-none border border-white/5 bg-[#0a0a0b] p-7 transition-colors hover:border-white/10")}>
              {/* Top accent bar */}
              <div className="absolute top-0 left-0 h-[2px] w-full bg-[#10b981] opacity-75" />
